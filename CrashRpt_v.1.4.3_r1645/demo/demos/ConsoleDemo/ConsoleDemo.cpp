...
int main(int argc, char* argv[])
{
	...
	CString sTmpFolder = Utility::GetModulePath(NULL);
	sTmpFolder += _T("\\Dump");
	Utility::CreateFolder(sTmpFolder);
	info.pszErrorReportSaveDir = sTmpFolder;

	info.pszEmailTo = _T("15901216901@139.com");      // Email recipient address
	info.pszSmtpProxy = _T("smtp.qq.com:465");  // Use SMTP proxy.
	info.pszSmtpLogin = _T("702054083@qq.com");      // SMTP Login
	info.pszSmtpPassword = _T("Qh410381");       // SMTP Password
